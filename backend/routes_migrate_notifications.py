...
def migrate_notifications_flash():
    ...
    try:
        ...
        sql_commands = [
            ...
            "ALTER TABLE members ADD COLUMN IF NOT EXISTS location_updated_at TIMESTAMP",
            ...
            # 5. OFFRES FLASH
            "ALTER TABLE offers ADD COLUMN IF NOT EXISTS is_flash BOOLEAN DEFAULT FALSE",
            "ALTER TABLE offers ADD COLUMN IF NOT EXISTS total_stock INTEGER DEFAULT 1",
            "ALTER TABLE offers ADD COLUMN IF NOT EXISTS current_stock INTEGER DEFAULT 1",
            "ALTER TABLE offers ADD COLUMN IF NOT EXISTS validity_start TIMESTAMP",
            "ALTER TABLE offers ADD COLUMN IF NOT EXISTS validity_end TIMESTAMP",
            ...
            # 6. RÉSERVATIONS D'OFFRES FLASH
            "ALTER TABLE bookings ADD COLUMN IF NOT EXISTS offer_id INTEGER REFERENCES offers(id) ON DELETE CASCADE",
        ...
        for command in sql_commands:
            try:
                ...
                results.append(f"✅ OK: {command[:60]}...")
            except Exception as e:
                error_msg = str(e)
                if "already exists" in error_msg or "duplicate" in error_msg.lower():
                    results.append(f"ℹ️ Déjà existant: {command[:60]}...")
                else:
                    results.append(f"⚠️ Erreur: {error_msg[:100]}")
