...
# ✅ IMPORT get_jwt POUR LIRE LES CLAIMS
...
print("🚀 DÉMARRAGE V11.4 (JWT STANDARD)...", file=sys.stdout)

app = Flask(__name__, static_folder='../frontend/dist')
CORS(app)

# 🔑 NOUVELLE CLÉ (Force la déconnexion des vieux tokens buggés)
app.config['SECRET_KEY'] = 'peps_v11_4_final_key_reset'
app.config['JWT_SECRET_KEY'] = 'peps_jwt_v11_4_key_reset'
...
# --- 🕵️ GESTIONNAIRES D'ERREURS JWT (TRANSFORME 422 EN MESSAGE CLAIR) ---
@jwt.invalid_token_loader
def invalid_token_callback(error):
    return jsonify({
        'error': 'Invalid Token',
        'message': f'Session invalide ou corrompue. ({error})'
    }), 401
...
# --- 🔐 HELPER AUTH CORRIGÉ (STANDARD) ---
def get_auth_user():
    """Récupère l'utilisateur via l'ID (String) stocké dans le token"""
    try:
        # V11.4 : L'identité est maintenant juste un ID (String)
        uid = get_jwt_identity()
        if not uid: return None
        # On convertit en int pour la DB
        return User.query.get(int(uid))
    except Exception as e:
        print(f"❌ Auth Error: {e}", file=sys.stdout)
        return None
...
# ==========================================
# 🛡️ ROUTES ADMIN (UTILISE get_jwt() POUR LE ROLE)
# ==========================================

@app.route('/api/admin/test-no-jwt')
def admin_test_no_jwt():
    return jsonify({"status": "ok", "version": "V11.4"})

@app.route('/api/admin/global-stats')
@jwt_required()
def admin_global_stats():
    # 1. Vérification via les CLAIMS (Rapide et Sûr)
    claims = get_jwt()
    role = claims.get('role')
    
    if role != 'admin': 
        return jsonify(error=f"Unauthorized (Role: {role})"), 403
...
# ==========================================
# 🔐 LOGIN / REGISTER (CRÉATION TOKEN STANDARD)
# ==========================================
...
def login():
    ...
    if u and u.role == 'member' and not u.is_both and dev_id:
        if UserDevice.query.filter_by(user_id=u.id).count() >= 2:
            return jsonify({'error': 'Max 2 appareils'}), 403
        if not UserDevice.query.filter_by(user_id=u.id, device_fingerprint=dev_id).first():
    ...
    if u and check_password_hash(u.password_hash, d.get('password')):
        # ✅ CORRECTION : Identity = String ID, Role = Claim
        token = create_access_token(
            identity=str(u.id), 
            additional_claims={'role': u.role, 'is_both': u.is_both}
        )
...
def register():
    ...
    token = create_access_token(
        identity=str(u.id), 
        additional_claims={'role': final_role, 'is_both': is_both}
    )
...
def health(): return jsonify({"status": "ONLINE", "version": "V11.4"})
...
def setup_v11():
    ...
    if not User.query.filter_by(email='partner@peps.swiss').first():
        ...
        db.session.add(u); db.session.commit()
        db.session.add(Partner(user_id=u.id, name="Partner Only", category="Restaurant", booking_enabled=True, latitude=47.13, longitude=7.24))
